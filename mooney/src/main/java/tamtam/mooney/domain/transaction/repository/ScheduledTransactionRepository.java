...
public interface ScheduledTransactionRepository extends JpaRepository<ScheduledTransaction, Long> {
    ...
    @Query("SELECT s FROM ScheduledTransaction s WHERE s.user = :user AND s.scheduledDate BETWEEN :startOfMonth AND :endOfMonth ORDER BY s.scheduledDate ASC")
    List<ScheduledTransaction> findByUserAndScheduledDateBetween(
